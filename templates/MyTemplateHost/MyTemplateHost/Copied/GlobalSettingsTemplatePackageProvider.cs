            ...
            var results = await Task.WhenAll(installRequests.Select(async installRequest =>
            {
                ...
                if (installersThatCanInstall.Count == 0)
                {
                    ...
                    return InstallResult.CreateFailure(
                        ...
                        string.Format("LocalizableStrings.GlobalSettingsTemplatePackageProvider_InstallResult_Error_PackageCannotBeInstalled. Message = {0}", installRequest.PackageIdentifier)
                        );
                }
                if (installersThatCanInstall.Count > 1)
                {
                    return InstallResult.CreateFailure(
                        ...
                        string.Format("LocalizableStrings.GlobalSettingsTemplatePackageProvider_InstallResult_Error_MultipleInstallersCanBeUsed. Message = {0}", installRequest.PackageIdentifier));
                }
        ...
        private async Task<(InstallerErrorCode, string)> EnsureInstallPrerequisites(List<TemplatePackageData> packagesInSettings, string identifier, string version, IInstaller installer, CancellationToken cancellationToken, bool update = false)
        {
            ...
            if (packages.OfType<IManagedTemplatePackage>().FirstOrDefault(s => s.Identifier == identifier && s.Installer == installer) is IManagedTemplatePackage packageToBeUpdated)
            {
                ...
                if (packageToBeUpdated.Version == version)
                {
                    ...
                    return (InstallerErrorCode.AlreadyInstalled, string.Format("LocalizableStrings.GlobalSettingsTemplatePackageProvider_InstallResult_Error_PackageAlreadyInstalled. Message ={0}", packageToBeUpdated.DisplayName));
                }
                if (!update)
                {
                ...
                _logger.LogInformation(
                    string.Format(
                        "LocalizableStrings.GlobalSettingsTemplatePackagesProvider_Info_PackageUninstalled. DisplayName={0}",
